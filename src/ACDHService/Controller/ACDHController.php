...
class ACDHController {
    ...
    public function post(Application $app, Request $request) {
        ...
        /* 
         * we need to create the resource without the transaction id, because after the
         * commitTransaction() function the resource uri contains the transaction ID
         * and then the metadata insterting will added to a wrong resource
         *   
         */
        $uri = $chullo->createResource(
            '', file_get_contents($file['path']), array('Content-Type' => $file['mime'])
        );
        ...
        foreach ($metadata as $key => $value) {
            // if the value starts with the http(s) then addResource else normal add
        ...
        // it is adding the available class names to the rdf:type section
        // for example:  http://localhost:8080/fcrepo/rest/tx:dcc6fd14-56dd-4d43-a39a-0a4e60369cf8/cb/69/67/6d/Collection
        ...
        */
        ...
        $result = $sparql->insert($rdf);
        var_dump($uri);
        
               
        /* select the new inserted data */
        $sparql = new \EasyRdf_Sparql_Client('http://127.0.0.1:8080/bigdata/sparql');
        $resultS = $sparql->query('SELECT * WHERE { <'.$uri.'> ?p ?o}');
        var_dump($resultS->dump());
